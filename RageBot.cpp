...
void CRageBot::Move(CUserCmd *pCmd, bool &bSendPacket)
{
	...
	// Anti Aim 
	if (options.Rage.AntiAim)
		DoAntiAim(pCmd, bSendPacket);

	// Position Adjustment
	if (true)//Menu::Window.RageBotTab.AccuracyPositionAdjustment.GetState()
		PositionAdjustment(pCmd);

	// Recoil
	if (options.Rage.DoNoRecoil)
		DoNoRecoil(pCmd);
...
void CRageBot::DoAimbot(CUserCmd *pCmd,bool &bSendPacket) // Creds to encore1337 for getting it to work
{
	...
	if (FindNewTarget)
	{
		...
		switch (options.Rage.TargetSelection)
		{
		case 0:
	...
	if (TargetID >= 0 && pTarget)
	{
		...
		// Key
		if (false)//Menu::Window.RageBotTab.AimbotKeyPress.GetState()
		{
	...
	//	}
		...
		float pointscale = options.Rage.TargetPointscale - 0.5f; // Aim height
//		float value = options.Rage.Hitchance; // Hitchance

		Vector Point;
		Vector AimPoint = GetHitboxPosition(pTarget, HitBox) + Vector(0, 0, pointscale);

		if (options.Rage.TargetMultipoint)
		{
		...
		// Lets open fire
		if (GameUtils::IsScopedWeapon(pWeapon) && !pWeapon->IsScoped() && options.Rage.AutoScope) // Autoscope
		{
			pCmd->buttons |= IN_ATTACK2;
		}
		else
		{
			if ((options.Rage.Hitchance * 1.5 <= hitchance(pLocal, pWeapon)) || options.Rage.Hitchance == 0 || *pWeapon->m_AttributeManager()->m_Item()->ItemDefinitionIndex() == 64)
			{
				if (AimAtPoint(pLocal, Point, pCmd, bSendPacket))
				{
					if (options.Rage.AutoFire&& !(pCmd->buttons & IN_ATTACK))
					{
				...
				else if (options.Rage.AutoFire&& !(pCmd->buttons & IN_ATTACK))
				{
					pCmd->buttons |= IN_ATTACK;
				}
			}
		...
		if (TargetID >= 0 && pTarget)
		{
			if (false)//Menu::Window.RageBotTab.AccuracyAutoStop.GetState()
			{
	...
	// Auto Pistol
	if (GameUtils::IsPistol(pWeapon) && options.Rage.AutoPistol)
	{
...
bool CRageBot::TargetMeetsRequirements(IClientEntity* pEntity)
{
	...
	if (pEntity && pEntity->IsDormant() == false && pEntity->IsAlive() && pEntity->GetIndex() != hackManager.pLocal()->GetIndex())
	{
		...
		{
			// Team Check
			if (pEntity->GetTeamNum() != hackManager.pLocal()->GetTeamNum() || false)//Menu::Window.RageBotTab.TargetFriendlyFire.GetState()
			{
				// Spawn Check
...
int CRageBot::GetTargetCrosshair()
{
	// Target selection
	int target = -1;
	float minFoV = options.Rage.AimbotFov;
	...
	for (int i = 0; i < Interfaces::EntList->GetMaxEntities(); i++)
	{
		...
		if (TargetMeetsRequirements(pEntity))
		{
			...
			if (NewHitBox >= 0)
			{
				...
				float fov = FovToPlayer(ViewOffset, View, pEntity, 0);
				if (Distance < minDist && fov < options.Rage.AimbotFov)
				{
					minDist = Distance;
					target = i;
				}
	...
	for (int i = 0; i < Interfaces::EntList->GetMaxEntities(); i++)
	{
		...
		if (TargetMeetsRequirements(pEntity))
		{
			...
			if (NewHitBox >= 0)
			{
				...
				float fov = FovToPlayer(ViewOffset, View, pEntity, 0);
				if (Health < minHealth && fov < options.Rage.AimbotFov)
				{
					minHealth = Health;
					target = i;
				}
...
#pragma region GetHitboxesToScan
	...
	bool Multipoint = options.Rage.TargetMultipoint;
...
#pragma endregion Get the list of shit to scan
	...
	for (auto HitBoxID : HitBoxesToScan)
	{
		if (AWall)
		{
			...
			{
				c = Color(0, 255, 0, 255);
				if (Damage >= options.Rage.MinimumDamage)
				{
					return HitBoxID;
				}
			}
...
bool CRageBot::AimAtPoint(IClientEntity* pLocal, Vector point, CUserCmd *pCmd, bool &bSendPacket)
{
	...
	if (fovLeft > 25.0f &&false)// Menu::Window.RageBotTab.AimbotAimStep.GetState()
	{
	...
	// Silent Aim
	if (options.Rage.SilentAim && !options.Rage.pSilentaim)
	{
...
namespace AntiAims // CanOpenFire checks for fake anti aims?
{
	...
	void Backward(CUserCmd* pCmd, bool &bSendPacket) {
		int random = rand() % 100;
	...
	void skeet(CUserCmd* pCmd, bool &bSendPacket) {
		...
		pCmd->viewangles[1] = fAngY + v28;
	}
	void skeeta(CUserCmd* pCmd, bool &bSendPacket) {
		if (pCmd->command_number % 2)
		{
			pCmd->viewangles.x = 69.0f;
			pCmd->viewangles.y = 180.0f;
		}
	}
	void aaa(CUserCmd* pCmd, bool &bSendPacket) {
		QAngle angle_for_yaw;
		IClientEntity* m_local = hackManager.localplayer();
		static int counter = 0;
		static int motion = 0;
		int ServerTime = (float)Interfaces::Globals->interval_per_tick * m_local->GetTickBase() * 2;
		bool flip = true;
		...
		if (flip)
		{
			...
			switch (value) {
			case 0:pCmd->viewangles.y = m_local->GetLowerBodyYaw() - 90.00f > 35 ? m_local->GetLowerBodyYaw() - 180.f : m_local->GetLowerBodyYaw() - 90.f; break;
				bSendPacket = false;

			case 1:pCmd->viewangles.y = m_local->GetLowerBodyYaw() + 90.00f > 35 ? m_local->GetLowerBodyYaw() - 90.f : m_local->GetLowerBodyYaw() + 90.f; break;
				bSendPacket = true;
			}
			counter++;
		}
...
bool CRageBot::freestanding(CUserCmd* m_pcmd, bool packet) {
	...
	if (false && m_local->GetVelocity().Length() < 300) {//enu::Window.RageBotTab.Freestanding.GetState(
...
void CRageBot::DoAntiAim(CUserCmd *pCmd, bool &bSendPacket) // pCmd->viewangles.y = 0xFFFFF INT_MAX or idk
{
	...
	if (!freestanding(pCmd, bSendPacket)) {
		...
		switch (options.Rage.AntiAimPtich) // Magic pitch is 69.69? /
		{
		case 0:
			break;
		case 1:
			...
			break;
		case 2:
			pCmd->viewangles.x = 72.f;
			break;
		}
			static bool bFlip;
		bFlip = !bFlip;
		bSendPacket = bFlip;
		switch (options.Rage.AntiAimYaw)//
		{
		case 0:
			break;
		case 1:
			AntiAims::Backward(pCmd, bSendPacket);
			break;
		case 2:
		...
		case 5:
		{
			AntiAims::skeeta(pCmd, bSendPacket);
			
		}
		break;
		case 6:
		{
			AntiAims::aaa(pCmd, bSendPacket);
		...
		case 7:
		{
			...
			if (!bSendPacket)
			{
				...
				if (fakeTick < 300)
					pCmd->viewangles.y += 67.57f;
				else if (fakeTick > 300)
					pCmd->viewangles.y -= 67.15f;
	...
	G::m_real_angle = pCmd->viewangles;
/*	if (bSendPacket) {
		G::m_real_angle = pCmd->viewangles;
	}
	else
	{
		G::m_fake_angle = pCmd->viewangles;
	}*/
	G::m_lowerbodyyaw = pLocal->GetLowerBodyYaw();
}
