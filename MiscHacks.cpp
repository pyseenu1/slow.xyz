...
{
	// Any Move Stuff
	if (options.Rage.MainSwitchmisc) {
		// Bhop
		if (options.Rage.bhop) {
			AutoJump(pCmd);
		}

		Fakelag(pCmd, bSendPacket);
		// AutoStrafe
		if (options.Rage.AutoStrafe) {
			RageStrafe(pCmd);
		}
...
{
	int m_nChoked = Menu::Window.RageBotTab.Fakelagvalue.GetValue() * 90;

	static int iFakeLag = -1;
	iFakeLag++;
	Vector m_vecOldPos;
	float flTeleportDistanceSqr = 64.f * 64.f;

	if (m_nChoked >= 1)
	{
		bSendPacket = true;
		m_nChoked = 0;
		m_vecOldPos = hackManager.localplayer()->GetOrigin();

		return;
	}

	Vector vDelta = m_vecOldPos - hackManager.localplayer()->GetOrigin();
