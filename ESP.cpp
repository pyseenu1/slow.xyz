...
void direction_arrow(const Vector& origin) {
	...
	float rotation = G::m_real_angle.y;
	...
	//visualconfig.player.cDirectionArrowColor
	Render::DrawRect(flXPosition, flYPosition, 10, 10, Color(255, 0, 0, 255));
	//	}
}
void CEsp::Draw()
{
	...
	if (options.Rage.DrawLinesAA && hackManager.pLocal()->IsAlive()) {
		CEsp::DrawLinesAA(Color);
	}

	if (options.Rage.SpecList)
	{
		SpecList();
	}
	...
	for (int i = 0; i < Interfaces::EntList->GetHighestEntityIndex(); i++)
	{
		...
		if (pEntity && !pEntity->IsDormant())
		{
			...
			if (options.Rage.DrawDrop && cClass->m_ClassID != (int)CSGOClassID::CBaseWeaponWorldModel && ((strstr(cClass->m_pNetworkName, "Weapon") || cClass->m_ClassID == (int)CSGOClassID::CDEagle || cClass->m_ClassID == (int)CSGOClassID::CAK47)))
			{
				DrawDrop(pEntity, cClass);
			}
			
			if (options.Rage.FiltersC4)
			{
			...
			if (options.Rage.FiltersChickens)
			{
	...
	for (int i = 0; i < Interfaces::EntList->GetHighestEntityIndex(); i++)
	{
		...
		if (m_entity->GetClientClass()->m_ClassID == ccsplayer) {
			...
			if (options.Rage.MainSwitchvh && Interfaces::Engine->GetPlayerInfo(i, &pinfo) && pEntity->IsAlive())
			{
...
void CEsp::DrawPlayer(IClientEntity* pEntity, player_info_t pinfo)
{
	...
	if (/*Menu::Window.VisualsTab.FiltersEnemiesOnly.GetState()*/true && (pEntity->GetTeamNum() == hackManager.pLocal()->GetTeamNum()))
		return;
	...
	if (GetBox(pEntity, Box))
	{
		...
		if (options.Rage.OptionsBox)
			DrawBox(pEntity, Box, Color);

		if (options.Rage.OptionsName)
			DrawName(pEntity, pinfo, Box);

		if (options.Rage.OptionsHealth)
		...
		if (options.Rage.DrawArmor)
			DrawArmor(pEntity, Box);

		if (options.Rage.OptionsSkeleton)
