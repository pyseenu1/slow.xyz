...
bool ResolverSetup::YawResolve2(IClientEntity *player)
{
	...
	int result; // al@1
	float viewangles = player->GetEyeAnglesXY()->y;
...
bool ResolverSetup::YawResolve3(IClientEntity *player)
{
	...
	if (bLowerBodyUpdated)
	{
		...
		if (bLowerBodyUpdated)
		{
			result = player->GetEyeAnglesXY()->y;

			if ((player->GetEyeAnglesXY()->y + 16 > player->GetEyeAnglesXY()->y + 16) + 90.0);
		}
		else
		{
			if ((player->GetEyeAnglesXY()->y + 16) > player->GetEyeAnglesXY()->y + 16) - 90.0;
...
bool ResolverSetup::YawResolve4(IClientEntity *player)
{
	...
	if (bLowerBodyUpdated)
	{
		...
		{
			if ((player->GetEyeAnglesXY()->y + 16 > player->GetEyeAnglesXY()->y + 16) - 180.0);
		}
	}
...
bool ResolverSetup::YawResolve5(IClientEntity *player)
{
	...
	if (v6 < 15)
	{
		v4 = (player->GetEyeAnglesXY()->y + 16) + (float)(rand() % 141 - 70);
		result = player->GetEyeAnglesXY()->y;
		if (player->GetEyeAnglesXY()->y + 16 == v4);
	}
	if (v6 == 69)
	{
		v5 = (player->GetEyeAnglesXY()->y + 16) + (float)(rand() % 181 - 90);
		result = player->GetEyeAnglesXY()->y;
		if (player->GetEyeAnglesXY()->y + 16 == v5);
	}
	return result;
}
bool ResolverSetup::YawResolve6(IClientEntity *player)
{
	...
	if ((player->GetEyeAnglesXY()->y + 16 > player->GetEyeAnglesXY()->y + 16) - 91.0);
...
bool ResolverSetup::YawResolve7(IClientEntity *player)
{
	...
	if (PlayerIsMoving)
	{
		...
		{
			result = oldlowerbodyyaw;
			if ((player->GetEyeAnglesXY()->y + 16 > player->GetEyeAnglesXY()->y + 16) + 0.0);
		}
		else
		{
			if ((player->GetEyeAnglesXY()->y + 16 > player->GetEyeAnglesXY()->y + 16) + 180.0);
...
bool ResolverSetup::YawResolve9(IClientEntity *player)
{
	...
	if (IsEntityMoving(player))
	{
		...
		{
			if (bLowerBodyUpdated)
				if ((player->GetEyeAnglesXY()->y + 16 > player->GetEyeAnglesXY()->y + 16) - 226.0);
				else
					if ((player->GetEyeAnglesXY()->y + 16 > player->GetEyeAnglesXY()->y + 16) - 134.0);
...
void ResolverSetup::Resolve(IClientEntity* pEntity)
{
	...
	/*if (options.Rage.AccuracyResolver)
	{
	...
	if (options.Rage.AccuracyResolver)
	{
		...
		{
			resolvokek::resolvemode == 1;
			pEntity->GetEyeAnglesXY()->y = pEntity->GetLowerBodyYaw() + YawResolve8(pEntity) + YawResolve9(pEntity) + YawResolve6(pEntity) + YawResolve7(pEntity) + YawResolve5(pEntity);
		}
		else
		{
			resolvokek::resolvemode == 6;
	...
	pEntity->GetEyeAnglesXY()->y = pEntity->GetLowerBodyYaw();
	movinglby[pEntity->GetIndex()] = pEntity->GetLowerBodyYaw() + lby_storage(pEntity);
	}
	else if (lbytomovinglbydelta[pEntity->GetIndex()] > 50 && lbytomovinglbydelta[pEntity->GetIndex()] < -50) {// the 50 will allow you to have a 30 degree margin of error (do the math :))
	pEntity->GetEyeAnglesXY()->y = movinglby[pEntity->GetIndex()];
	}
	else
	pEntity->GetEyeAnglesXY()->y = pEntity->GetLowerBodyYaw() + lby_storage(pEntity);
